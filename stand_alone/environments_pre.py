...
class RodentSingleClipTrack(PipelineEnv):
    ...
    def reset(self, rng) -> State:
        ...
        info = {"start_frame": start_frame, "next_frame": start_frame + 1}
    ...
    def _calculate_termination(self, state, ref) -> bool:
        ...
        bpos_c = (
            data_c.xpos
        )  # is xpos the same with bpos? 54 (expert) compare to 66 (agent)
        ...
        return (
            ...
            - (1 / 0.3)
            * (
                (jp.linalg.norm(bpos_c - (bpos_ref)))
                + (jp.linalg.norm(qpos_c - (qpos_ref)))
            )
            < 0
        )
        ...
        def f(x):
            if len(x.shape) == 2:
                return jax.lax.dynamic_slice_in_dim(
                    x, start_frame, self._ref_traj_length, axis=1
                )
