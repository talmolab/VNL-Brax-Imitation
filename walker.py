"""
MJX-compatible Rodent walker.
This one uses an xml that already includes a floor
"""
...
_XML_PATH = os.path.join(os.path.dirname(__file__), "assets/rodent.xml")
...
class Rat(legacy_base.Walker):
    ...
    def _build(
        ...
        if foot_mods:
            self._mjcf_root.find("default", "ankle").joint.range = [-0.1, 2.0]
    ...
    def mocap_joints(self):
        return tuple(self._mjcf_root.find("joint", name) for name in _RAT_MOCAP_JOINTS)

    @composer.cached_property
    def mocap_joint_order(self):
        return tuple(
            [jnt.name for jnt in self.mocap_joints]
        )  #  This lint is mistaken; pylint: disable=not-an-iterable
    ...
    def primary_joints(self):
        ...
        return tuple(
            jnt
            for jnt in self._mjcf_root.find_all("joint")
    ...
    def vertebra_joints(self):
        ...
        return tuple(
            jnt for jnt in self._mjcf_root.find_all("joint") if "vertebra" in jnt.name
        )
    ...
    def primary_joint_order(self):
        ...
        primary_names = tuple(
            [jnt.name for jnt in self.primary_joints]
    ...
    def vertebra_joint_order(self):
        joint_names = self.mocap_joint_order
        vertebra_names = tuple(
            [jnt.name for jnt in self.vertebra_joints]
        )  # pylint: disable=not-an-iterable
        vertebra_order = []
    ...
    def joint_actuators(self):
        ...
        return tuple([act for act in self._mjcf_root.find_all("actuator") if act.joint])
    ...
    def pose_to_actuation(self, pose):
        ...
        r_lower = np.array(
            [ajr[0] for ajr in self.joint_actuators_range]
        )  #  This lint is mistaken; pylint: disable=not-an-iterable
        r_upper = np.array(
            [ajr[1] for ajr in self.joint_actuators_range]
        )  #  This lint is mistaken; pylint: disable=not-an-iterable
        ...
        return np.hstack(
            [
                tendon_actions,
                (2 * pose[self.joint_actuator_order] - (r_lower + r_upper))
                / (r_upper - r_lower),
            ]
        )
    ...
    def joint_actuator_order(self):
        ...
        joint_actuator_names = tuple(
            [act.name for act in self.joint_actuators]
        )  #  This lint is mistaken; pylint: disable=not-an-iterable
        actuator_order = []
