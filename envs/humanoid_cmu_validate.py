...
class HumanoidTracking(PipelineEnv):
    ...
    def _calculate_termination(self, state) -> float:
        ...
        xpos_flatten = data_c.xpos[
            self._body_locations
        ].flatten()  # jp.broadcast_to(data_c.xpos[self._body_locations], self.ref_traj.body_positions.shape)
        ...
        error_bodies = jp.mean(jp.abs((target_bodies - xpos_flatten)))
    ...
    def get_reference_rel_bodies_pos_local(self, data, ref_traj):
        ...
        xpos_flatten = data.xpos[
            self._body_locations
        ].flatten()  # jp.broadcast_to(data.xpos[self._body_locations], ref_traj.body_positions.shape)

        # print(xpos_flatten[:10])

        obs = self.global_vector_to_local_frame(
            data,
            (ref_traj.body_positions - xpos_flatten).reshape(
                [self._ref_traj_length, 30, 3]
            ),
        )
    ...
    def get_reference_rel_bodies_pos_global(self, data, ref_traj):
        ...
        diff = ref_traj.body_positions - xpos_flatten
