...
def env_setup(params):
    ...
    end_eff_idx = jp.array(
        [int(axis.convert_key_item(key)) for key in params["end_eff_names"]]
    )
...
class RodentSingleClipTrack(PipelineEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        mj_model, self._end_eff_idx, self.body_idxs, self.joint_order = env_setup(
            params
        )
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        total_reward = (
            0.01 * rcom + 0.01 * rvel + 0.01 * rapp + 0.01 * rquat + 0.0001 * ract
        )
    ...
    def _calculate_termination(self, state) -> float:
        ...
        target_bodies = self._ref_traj.body_positions[
            state.info["cur_frame"], :
        ]  # (18 (spots) x 3 dimension(x,y,z))
        error_bodies = jp.mean(jp.abs((target_bodies - data_c.xpos[self.body_idxs])))
        termination_error = (1 / 0.3) * (
            jp.sum(jp.abs(error_bodies)) + jp.sum(jp.abs(error_joints))
        )
    ...
    def get_reference_rel_bodies_pos_local(self, data, ref_traj):
        ...
        obs = self.global_vector_to_local_frame(data, diff[:, self.body_idxs])
