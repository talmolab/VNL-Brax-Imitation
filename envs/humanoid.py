...
class HumanoidTracking(PipelineEnv):
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        # total_reward = rcom + rvel + rtrunk + rquat + ract + rapp + is_healthy * 0.5 # add healhty reward
        total_reward = rcom + is_healthy * 0.5 # simplified the reward composition
    ...
    def _calculate_termination(self, state) -> float:
        ...
        is_healthy = jp.where(data_c.x.pos[0, 2] < min_z, 0.0, 1.0)
        is_healthy = jp.where(data_c.x.pos[0, 2] > max_z, 0.0, is_healthy)
